...
gamma_flat = 0;
info_gauss_all = zeros(length(trialsSweep), 4);
info_binned_all = zeros(length(trialsSweep), 1);
...
for a=1:length(alphasweep)
    ...
    parfor b=1:length(Bsweep)
        ...
        rate1 = B + alpha*beta*[0,1,0,1,2,2,0,3,3,0,4,4] + alpha*(1-beta)*[0,0,1,1,2,0,2,3,0,3,4,0];
        rate2 = B + alpha*beta*[0,0,1,1,2,0,2,3,0,3,4,0] + alpha*(1-beta)*[0,1,0,1,2,2,0,3,3,0,4,4];
        X1_tmp=[];
        X2_tmp=[];
        Y = [];
        for rateIdx = 1:length(rate1)
            ...
            Y      =[Y, rateIdx*ones(1,num_trials)];
        end
        X1(a,b,:) = X1_tmp;
        X2(a,b,:) = X2_tmp;
        I_gt(a,b) = groundtruthinformation(rate1, rate2, 1/12, num_stimuli, 100);
    end
end
Y =[ones(1,num_trials) 2*ones(1,num_trials) 3*ones(1,num_trials) 4*ones(1,num_trials), ...
    5*ones(1,num_trials) 6*ones(1,num_trials) 7*ones(1,num_trials) 8*ones(1,num_trials), ...
    9*ones(1,num_trials) 10*ones(1,num_trials) 11*ones(1,num_trials) 12*ones(1,num_trials)];
save('Discrete/SimulationR1/R1A_B/I_gt_12Stim.mat', "I_gt" );
save('Discrete/SimulationR1/R1A_B/data_12Stim.mat', 'X1', 'X2', 'Y', 'Bsweep', 'alphasweep');
%% Analysis Simulation
load('data_12Stim.mat');
opts_MI.bias='plugin';
...
save('Discrete/SimulationR1/R1A_B/I_discrete_12S.mat', 'I_discrete');
%% Plot
clc, clear, close all
greyColors = [ ...
    255, 255, 255;  
...
% Plot
load('Discrete/SimulationR1/R1A_B/I_discrete_12S.mat', 'I_discrete');
info_venk = load('info_Gpid_12Stim.mat');
load('I_gt_12Stim.mat');
load('data_12Stim.mat');
...
saveas(figure_handle, 'Combined_Info_Map_12Stim.svg');
%% Helper Function
